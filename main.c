...
static uint32_t sub_word(uint8_t *arr)
{
    uint32_t y;
    for(int i=0;i<4;++i)
        *(arr+i)=sbox[*(arr+i)];
    y=(*(arr+0)<<24)|(*(arr+1)<<16)|(*(arr+2)<<8)|*(arr+3);
    return y;
}

static void key_generate(void)
{
    srand(time(NULL));
    for(int i=0;i<16;++i)
        *(key+i)=((rand()%16)<<4)|(rand()%16);
    for(int i=0;i<4;++i)
    ...
    for(int i=4;i<44;++i)
    {
        ...
        if(i%4==0)
        {
            uint8_t con_temp_to_array[4];
            for(int i=0;i<4;++i)
                *(con_temp_to_array+i)=temp>>((i+1)*(24-8*i));
            rot_word(con_temp_to_array);
            temp=sub_word(con_temp_to_array)^RCon[(int)(i/4)];
        }
...
static void round_key_construction(void)
{
    for(int i=0;i<11;++i)
        for(int j=4*i;j<4*i+4;++j)
            for(int k=0;k<4;++k)
                round_key[i][(j%4)*4+k]= words[j]>>(24-8*k);
}
